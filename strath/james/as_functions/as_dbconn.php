...
class As_Dbconn {
	...
	public function __construct()
	{
        ...
        if( $this->link->connect_errno )
        {
			...
			die(AS_I_TOP.'START SETTING UP THINGS'.AS_I_TOP_A.'<form action="'.as_database_setup().'" method="post">
             ...
             <tr><td>Database Username:</td><td><input type="text" name="username" autocomplete="off" required></td></tr>
    ...
    {
        //Delete clauses require a where param, otherwise use "trunclasse"
        if( empty( $where ) )
        {
            return false;
            exit;
    ...
    public function trunclasse( $tables = array() )
    {
        if( !empty( $tables ) )
        {
            $trunclassed = 0;
            ...
            {
                $trunclasse = "TRUNCATE TABLE `".trim($table)."`";
                $this->link->query( $trunclasse );
                if( !$this->link->error )
                {
                    $trunclassed++;
                }
