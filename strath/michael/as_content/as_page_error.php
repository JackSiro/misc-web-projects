<?php 
/*
	File: as_content/as_page_error.php
	Description: page for handling errors from as_install.php file when check for system core functionalities
...
$hidden = array();
	...
	if (as_clicked('DatabaseSetup')) {
		...
		if (empty($fielderrors)) {
			...
			$lines[12] = '	define( "AS_DB", 				"'.$database.'" );		// database name';
			$lines[13] = '	define( "AS_USER",				"'.$username.'" );			// database username';
			$lines[14] = '	define( "AS_PASS",				"'.$password.'"  );		// database password';
			$lines[15] = '	define( "AS_ERROR_RECEIVER",	"'.$emailto.'"  );		// email to receive error logs';
			$lines[16] = '	define( "AS_ERROR_SENDER",		"'.$emailfrom.'"  );	// email to send error logs';
