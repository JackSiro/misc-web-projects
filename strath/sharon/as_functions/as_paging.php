<?php
	...
	function my_stock_cart($cartno) {
		...
		if( $database->my_num_rows( $my_db_query ) > 0 ) {
                    list( $catid, $item_name, $item_image) = $database->get_row( $my_db_query );
		    return $item_image;
		} else  {
		    return false;
		}
    ...
    function my_item_items(){
		...
		{
		    	return '<option value="'.$row['catid'].'">'.$row['item_image'].'</option>';		                            
		}			
	}
	...
	function my_latest_item_items_home(){
		...
		foreach( $stock_itemids as $cat )
		{
			...
			if ($my_item_items_query) {
				echo '<hr><h3>'.$cat['item_image'].'</h3>
				   <div class="row">
					<div class="productsrow">';
			}	
				$database = new As_Dbconn();
				
				$item_field = $database->get_results( $my_item_items_query );
				foreach( $item_field as $row )
				{
	...
	function my_home_categories(){
		...
		$results = $database->get_results( $my_db_query );
		foreach( $results as $row ) {
			echo '<a href="'.my_siteLynk().$row['item_name'].'" >
			<div class="item_lynk"><img class="item_content" src="'.my_siteLynk_icon().$row['item_content'].'"/>
			'.$row['item_image'].'</div></a>';
	   }				
	}

	function my_lookup_cat($request){
		$my_db_query = "SELECT * FROM my_item WHERE item_name = '$request'";
		$database = new As_Dbconn();
