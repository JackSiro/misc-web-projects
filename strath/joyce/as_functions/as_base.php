<?php
	...
	function as_is_logged_official(){
		if (as_logged_official('username')) return true;
		else if (!as_logged_official('username')) return false;
	}
	
	function as_logged_admin(){
		if (as_is_logged()) {
			if (as_logged_official('level') == "admin") {
				return true;
			} else return false;
		}
	...
	if (!as_check_db_value('officialid', 'official_group', 'super-admin', 'as_official'))  {
		$as_err = array();
		$as_err['errno'] = 2;
		$as_err['errtype'] = 'user';
