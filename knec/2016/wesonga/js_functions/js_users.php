<?php
	function js_signin() {
		...
		$loginkey = md5($_POST['password']);
			...
			$_SESSION['itemsz_user'] = js_let_me_user($loginname, $loginkey);
			$_SESSION['itemsz_account'] = js_logged_account($loginname);
	...
	function js_is_logged(){
		$myloginid = isset( $_SESSION['itemsz_user'] ) ? $_SESSION['itemsz_user'] : "";
...
function logout() {
  unset( $_SESSION['itemsz_user'] );
  unset( $_SESSION['itemsz_account'] );
  header( "Location: index.php" );
}
	...
	function js_edit_user($js_userid){
		...
		$where_clause = array('userid' => $js_userid);
		$updated = $database->js_update( 'js_user', $Update_User_Details, $where_clause, 1 );
		if( $updated )	{	}
	}
