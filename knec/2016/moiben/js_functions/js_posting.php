<?php
	...
	function js_add_new_session(){
		...
		$add_query = $database->js_insert( 'js_session', $New_Session_Details ); 
	}
	
	function js_add_new_staff(){
		$database = new Js_Dbconn();			
		$New_Session_Details = array(			
			'staff_fullname' => trim($_POST['fullname']),
			'staff_mobile' => trim($_POST['mobile']),
			'staff_station' => trim($_POST['station']),
			'staff_task' => trim($_POST['task']),
			'staff_joined' => date('Y-m-d H:i:s'),
		);
			
		$add_query = $database->js_insert( 'js_staff', $New_Session_Details ); 
	}
