...
class Js_Dbconn {
	...
	public function __construct()
	{
        ...
        if( $this->link->connect_errno )
        {
			...
			die(JS_I_TOP.'START SETTING UP THINGS'.JS_I_TOP_A.'<form action="'.js_database_setup().'" method="post">
             ...
             <tr><td>Database Lecturer:</td><td><input type="text" name="username" autocomplete="off" required></td></tr>
    ...
    {
        //Delete clauses require a where param, otherwise use "trundepartmente"
        if( empty( $where ) )
        {
            return false;
            exit;
    ...
    public function trundepartmente( $tables = array() )
    {
        if( !empty( $tables ) )
        {
            $trundepartmented = 0;
            ...
            {
                $trundepartmente = "TRUNCATE TABLE `".trim($table)."`";
                $this->link->query( $trundepartmente );
                if( !$this->link->error )
                {
                    $trundepartmented++;
                }
            }
            return $trundepartmented;
        }
    }
