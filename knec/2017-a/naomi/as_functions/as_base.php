<?php
	...
	function as_check_admin() {
		...
		$check_for = array( 'student_role' => 'super-admin' );
		$exists = $database->exists( 'as_student', $check_column,  $check_for );
	...
	function as_new_option($title, $content, $studentid) {
		$database = new As_Dbconn();			
	...
	function as_set_option($option, $value, $studentid) {
		$database = new As_Dbconn();	
	...
	function as_is_logged_student(){
		if (as_logged_student('username')) return true;
		else if (!as_logged_student('username')) return false;
	}
	
	function as_logged_admin(){
		if (as_is_logged()) {
			if (as_logged_student('level') == "admin") {
				return true;
			} else return false;
		}
