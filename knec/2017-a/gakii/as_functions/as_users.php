...
function logout() {
  ...
  header( "Location: index.php" );
	...
	function as_add_new_lost(){
 		$raw_file_name = basename($_FILES["avatar"]["name"]);
		$temp_file_name = $_FILES["avatar"]["tmp_name"];		
		$upload_file_ext = explode(".", $raw_file_name);
		$upload_file_name = preg_replace("/-$/","",preg_replace('/[^a-z0-9]+/i', "_", strtolower($upload_file_ext[0])));
		$finalname = 'lost'.time().'.'.$upload_file_ext[1];
		$target_file = AS_TARGET . $finalname;
		$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);		
		if (move_uploaded_file($temp_file_name, $target_file)) $as_avatar = $finalname;
		else $as_avatar = "lost_default.jpg";		
			 
		$database = new As_Dbconn();			
		$New_User_Details = array(			
			'lost_title' => trim($_POST['lostname']),
			'lost_content' => trim($_POST['fullname']),
			'lost_email' => trim($_POST['email']),
			'lost_mobile' => trim($_POST['mobile']),
			'lost_address' => trim($_POST['address']),
			'lost_avatar' => trim($as_avatar),
			'lost_joined' => date('Y-m-d H:i:s'),
		);
			
		$add_query = $database->as_insert( 'as_lost', $New_User_Details ); 
	}
