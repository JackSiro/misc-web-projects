<?php 
	...
	list( $stockid, $stock_drugid, $stock_unit, $stock_postedby, $stock_posted, $stock_client, $stock_quantity, $stock_unit, $stock_img, $stock_updated, $stock_updatedby) = $database->get_row( $as_db_query );
}
	  ...
	  <div id="content"> 
        ...
        <div class="content_item">
			...
			<div class="main_content">
				...
				<h1> Drug: <?php echo as_drug_drug($stock_drugid) ?></h1> 
          <br><hr>
				<table>
				<tr><td>
				<center><img src="<?php echo "as_media/".$stock_img ?>" class="iconic_big_i"/></center>
				</td><td>
				<h2>Drug: <?php echo as_drug_drug($stock_drugid) ?></h2>
				<h2>Quantity: <?php echo $stock_unit.' '.$stock_unit ?>
				<?php echo ', Kshs. '.$stock_quantity.' each' ?></h2>
				<h2>Client: <?php echo as_client_drug($stock_client) ?></h2>
				<h2>Received on: <?php echo date("j/m/y", strtotime($stock_posted)); ?></h2>
				</td></tr>
				</table>
				<hr>
				<center><h2><a href="index.php?action=stock_edit&&as_stockid=<?php echo $stockid ?>">Edit this Drug</a>
				 | <a href="index.php?action=stock_delete&&as_stockid=<?php echo $stockid ?>" onclick="return confirm('Are you sure you want to delete this drug from the system? \nBe careful, this action can not be reversed.')">Delete this Drug</a></h2></center>
				...
				<input type="hidden" name="drugprice" value="<?php echo $stock_quantity ?>" />
				<input type="hidden" name="drugtitle" value="<?php echo $stock_unit." of ".as_drug_drug($stock_drugid) ?>" />
				<div class="detail_info">
					<br><br>
					<h1>Place an Prescription for this Drug</h1> 
					  <br><hr><br>
					...
					<br>
                        <center>
						<input type="submit" class="submit_this" name="PrescriptionNow" value="Prescription this Drug">
			  </center><br>
				</form>
				</div>
			</div>
