...
switch ( $action ) {
	...
	case 'rating_new':  rating_new();
		break;
	case 'worker_all': worker_all();
		break;
	case 'worker_new':  worker_new();
		break;
	case 'worker_view': worker_view();
		break;
	case 'worker_edit':  worker_edit();
		break;
	case 'worker_delete':  worker_delete();
		break;
	case 'profile': 
...
function as_homepage() {
	...
	if ( isset( $_POST['SubmitRates'] ) ) {
		as_submit_rates();
		header( "Location: index.php?action=ratings_all");
	}  else {
