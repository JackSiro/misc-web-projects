<?php
	function as_signin() {
		...
		if ( isset( $_POST['SignMeIn'] ) ) {
            ...
            if (as_let_me_user($loginname, $loginkey)){
				$_SESSION['kmdnwdems_user'] = as_let_me_user($loginname, $loginkey);
				$_SESSION['kmdnwdems_account'] = as_logged_account($loginname);
	...
	function as_is_logged(){
		$myloginid = isset( $_SESSION['kmdnwdems_user'] ) ? $_SESSION['kmdnwdems_user'] : "";
	...
	function as_signin_modal() {
	  if ( isset( $_POST['LetMeIn'] ) ) {
		...
		if( $database->as_num_rows( $as_db_query ) > 0 ) {
		    ...
		    $_SESSION['kmdnwdems_user'] = $userid;			
...
function logout() {
  unset( $_SESSION['kmdnwdems_user'] );
  unset( $_SESSION['kmdnwdems_account'] );
  header( "Location: index.php" );
}
	...
	function as_add_new_item(){
		...
		$upload_file_ext = explode(".", $raw_file_name);
		$upload_file_name = preg_replace("/-$/","",preg_replace('/[^a-z0-9]+/i', "_", strtolower($upload_file_ext[0])));
		$finalname = 'item'.time().'.'.$upload_file_ext[1];
		$target_file = AS_TARGET . $finalname;
		$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);		
		if (move_uploaded_file($temp_file_name, $target_file)) $as_avatar = $finalname;
		else $as_avatar = "item_default.jpg";		
			 
		$database = new As_Dbconn();			
		$New_User_Details = array(			
			'item_name' => trim($_POST['itemname']),
			'item_code' => trim($_POST['fullname']),
			'item_unit' => trim($_POST['email']),
			'item_mobile' => trim($_POST['mobile']),
			'item_address' => trim($_POST['address']),
			'item_avatar' => trim($as_avatar),
			'item_created' => date('Y-m-d H:i:s'),
		);
			
		$add_query = $database->as_insert( 'as_item', $New_User_Details ); 
	}
