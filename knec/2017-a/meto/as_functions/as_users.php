<?php
	function as_signin() {
		...
		if ( isset( $_POST['SignMeIn'] ) ) {
            ...
            if (as_let_me_user($loginname, $loginkey)){
				$_SESSION['joyful_user'] = as_let_me_user($loginname, $loginkey);
				$_SESSION['joyful_account'] = as_logged_account($loginname);
	...
	function as_is_logged(){
		$myloginid = isset( $_SESSION['joyful_user'] ) ? $_SESSION['joyful_user'] : "";
...
function logout() {
  unset( $_SESSION['joyful_user'] );
  unset( $_SESSION['joyful_account'] );
  header( "Location: index.php" );
}
