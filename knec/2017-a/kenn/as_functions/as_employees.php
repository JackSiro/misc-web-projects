<?php
	function as_signin() {
		...
		$loginkey = md5($_POST['password']);
			...
			$_SESSION['mysite_user'] = as_let_me_user($loginname, $loginkey);
			$_SESSION['mysite_account'] = as_logged_account($loginname);
	...
	function as_is_logged(){
		$myloginid = isset( $_SESSION['mysite_user'] ) ? $_SESSION['mysite_user'] : "";
	...
	function as_signin_modal() {
	  if ( isset( $_POST['LetMeIn'] ) ) {
		...
		if( $database->as_num_rows( $as_db_query ) > 0 ) {
		    ...
		    $_SESSION['mysite_user'] = $employeeid;			
...
function logout() {
  unset( $_SESSION['mysite_user'] );
  unset( $_SESSION['mysite_account'] );
  header( "Location: index.php" );
}
