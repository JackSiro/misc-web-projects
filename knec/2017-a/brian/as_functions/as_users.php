<?php
	...
	function as_add_new_user(){
		...
		$imageFileDrug = pathinfo($target_file,PATHINFO_EXTENSION);		
	...
	function as_edit_user($userid){
		$database = new As_Dbconn();			
		$Item_User_Details = array(			
			'user_name' => trim($_POST['username']),
			'user_fname' => trim($_POST['fname']),
			'user_surname' => trim($_POST['surname']),
			'user_email' => trim($_POST['email']),
		...
		$where_clause = array('userid' => $userid);
		$updated = $database->as_update( 'as_user', $Item_User_Details, $where_clause, 1 );
		if( $updated )	{	}	
	}
