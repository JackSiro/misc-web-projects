...
function logout() {
  ...
  header( "Location: index.php" );
	...
	function as_add_new_worker(){
		...
		$upload_file_ext = explode(".", $raw_file_name);
		$upload_file_name = preg_replace("/-$/","",preg_replace('/[^a-z0-9]+/i', "_", strtolower($upload_file_ext[0])));
		$finalname = 'worker'.time().'.'.$upload_file_ext[1];
		$target_file = AS_TARGET . $finalname;
		$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);		
		if (move_uploaded_file($temp_file_name, $target_file)) $as_avatar = $finalname;
		else $as_avatar = "worker_default.jpg";		
			 
		$database = new As_Dbconn();			
		$New_User_Details = array(			
			'worker_name' => trim($_POST['workername']),
			'worker_fullname' => trim($_POST['fullname']),
			'worker_email' => trim($_POST['email']),
			'worker_mobile' => trim($_POST['mobile']),
			'worker_address' => trim($_POST['address']),
			'worker_avatar' => trim($as_avatar),
			'worker_joined' => date('Y-m-d H:i:s'),
		);
			
		$add_query = $database->as_insert( 'as_worker', $New_User_Details ); 
	}
