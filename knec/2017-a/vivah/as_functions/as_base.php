<?php
	...
	function as_check_admin() {
		$database = new As_Dbconn();
		$check_column = 'facilitatorid';
		$check_for = array( 'facilitator_group' => 'manager' );
		$exists = $database->exists( 'as_facilitator', $check_column,  $check_for );
	...
	function as_new_option($title, $content, $facilitatorid) {
		$database = new As_Dbconn();			
	...
	function as_set_option($option, $value, $facilitatorid) {
		$database = new As_Dbconn();	
		$Update_Site_Details = array(
			'content' => $value,
			'updatedby' => $facilitatorid,
	...
	function as_new_admin(){
	      if ( isset( $_POST['SetAdministrator'] ) ) {			
			...
			$New_User_Details = array(		
    				...
    				'facilitator_password' => md5(trim($_POST['password'])),
    				'facilitator_email' => trim($_POST['email']),
    				'facilitator_group' => 'manager',
    				'facilitator_organization' => 'facilitator_default.jpg',
	...
	function as_is_logged_facilitator(){
		if (as_logged_facilitator('username')) return true;
		else if (!as_logged_facilitator('username')) return false;
	}
	
	function as_logged_admin(){
		if (as_is_logged()) {
			if (as_logged_facilitator('level') == "admin") {
				return true;
			} else return false;
		}
