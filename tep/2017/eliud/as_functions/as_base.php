<?php
	...
	function as_check_admin() {
		$database = new As_Dbconn();
		$check_column = 'doctorid';
		$check_for = array( 'doctor_group' => 'Doctor' );
		$exists = $database->exists( 'as_doctor', $check_column,  $check_for );
	...
	function as_new_option($title, $content, $doctorid) {
		$database = new As_Dbconn();			
	...
	function as_set_option($option, $value, $doctorid) {
		$database = new As_Dbconn();	
	...
	function as_new_admin(){
	      if ( isset( $_POST['SetAdministrator'] ) ) {			
			...
			$New_User_Details = array(		
    				...
    				'doctor_group' => 'Doctor',
    				'doctor_avatar' => 'doctor_default.jpg',
